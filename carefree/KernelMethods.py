...
class KP:
    def __init__(self):
        self._x = self._alpha = self._b = self._kernel = None
    ...
    def fit(self, x, y, kernel='poly', p=None, gamma=None, lr=0.001, batch_size=128, epoch=10000, plot=True):
...
def test0():
    from carefree.util import gen_two_clusters, visualize2d
    xc, yc = gen_two_clusters()
    kp = KP()
    kp.fit(xc, yc, p=1)
    print('Accuracy: {}'.format((kp.predict(xc) == yc).mean() * 100))
